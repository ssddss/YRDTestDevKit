...
- (NSString *)YRD_uuid
{
    NSString *key = @"YRDUUID";
    NSString *uuid = [[NSUserDefaults standardUserDefaults] objectForKey:key];
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self YRD_createUUID] forKey:key];
...
- (NSString *) YRD_udid
{
    NSString *udid = [[YRDUDIDGenerator sharedInstance] UDID];
    if (udid.length==0) {
        udid = [self YRD_uuid];
        [[YRDUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *) YRD_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] YRD_macaddress];
        macid = [macaddress YRD_md5];
        if (!macid){
